...
import (
	...
	"pamoja-build/backend/internal/router"
)
...
func seedKeyholders() error {
	...
	var count int
	err := db.DB.QueryRow("SELECT COUNT(*) FROM keyholders").Scan(&count)
	if err != nil {
		return err
	}
	...
	for _, phone := range keyholderPhones {
		...
		var userID int64
		err := db.DB.QueryRow("SELECT id FROM users WHERE phone = ?", phone).Scan(&userID)
		if err != nil {
			...
			_, err = db.DB.Exec("UPDATE users SET role = 'keyholder' WHERE id = ?", userID)
			if err != nil {
		...
		_, err = db.DB.Exec("INSERT INTO keyholders (user_id) VALUES (?)", userID)
		if err != nil {
